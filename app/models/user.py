...
friends = db.Table(
    "friends",
    db.Column('requester_id', db.Integer, db.ForeignKey(add_prefix_for_prod('users.id'))),
    db.Column('accepter_id', db.Integer, db.ForeignKey(add_prefix_for_prod('users.id'))),
...
class User(db.Model, UserMixin):
    ...
    Sender = db.relationship('Transaction', backref='send',
                             lazy='dynamic', foreign_keys='Transaction.sender')
    Receiver = db.relationship('Transaction', backref='receive',
                               lazy='dynamic', foreign_keys='Transaction.receiver')

    Paymentdetails = db.relationship(
        'PaymentDetail', backref='paymentdetails', lazy='dynamic', foreign_keys='PaymentDetail.user_id')
    rel_comment = db.relationship(
        'Comment', backref='comm', lazy='dynamic', foreign_keys='Comment.user_id')
    ...
    def to_dict_friends(self):
        pending_requests = self.Friends.filter(
            friends.c.accepted == False).all()
        list_of_friends = self.Friends.filter(friends.c.accepted == True).all()
        list_of_friends2 = self.friends.filter(
            friends.c.accepted == True).all()
        friend_requests = self.friends.filter(
            friends.c.accepted == False).all()
