    ...
    if form.validate_on_submit():
        ...
        if user.profileImage != data['image'] and data['image']:
            ...
            if file and allowed_file(file.filename):
                ...
                profileImage = upload_file_to_s3(
...
def remove_payment():
    ...
    print(form.data)
    if form.validate_on_submit():
        paymentdetail = PaymentDetail.query.filter(
            PaymentDetail.user_id == form.data['user_id'], PaymentDetail.id == form.data['payment_detail_id']).delete()
        db.session.commit()
